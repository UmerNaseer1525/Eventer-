...
function MiniBar({ label, count, total, color }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
      <span style={{ width: 90, fontSize: 12, color: "#555", flexShrink: 0 }}>{label}</span>
      <Progress
        percent={pct}
        showInfo={false}
        strokeColor={color}
      ...
      />
      <span style={{ width: 28, fontSize: 12, fontWeight: 700, color, textAlign: "right" }}>{count}</span>
    </div>
  );
}
...
function ActivityItem({ event, index }) {
  ...
  return (
    ...
    >
      <Avatar
        size={38}
        style={{ background: color + "20", color, flexShrink: 0, fontWeight: 700 }}
      >
...
function Dashboard() {
  ...
  const upcomingEvents = validEvents.filter((e) => e.status.toLowerCase() === "upcoming");
  const ongoingEvents  = validEvents.filter((e) => e.status.toLowerCase() === "ongoing");
  const completedEvents= validEvents.filter((e) => e.status.toLowerCase() === "completed");
  const cancelledEvents= validEvents.filter((e) => e.status.toLowerCase() === "cancelled");
  ...
  const catColors = [C.blue, C.purple, C.orange, C.cyan, C.green, C.gold, C.red];

  // Recent 6 events for activity feed
  const recentEvents = useMemo(() => [...validEvents].slice(0, 6), [validEvents]);

  // Quick metric cards
  const quickMetrics = [
    { label: "Upcoming Events",  value: upcomingEvents.length,  icon: "🗓️", color: C.blue   },
    { label: "Ongoing Events",   value: ongoingEvents.length,   icon: "🔴", color: C.green  },
    { label: "Completed Events", value: completedEvents.length, icon: "✅", color: C.purple },
    { label: "Cancelled Events", value: cancelledEvents.length, icon: "❌", color: C.red    },
  ];
  ...
  return (
    <div style={{ padding: "10px" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#1a1a2e" }}>Dashboard</h1>
        ...
        {quickMetrics.map((m) => (
          <Col key={m.label} xs={12} sm={12} md={6}>
            ...
            >
              ...
              <div style={{ fontSize: 22, fontWeight: 900, color: m.color, margin: "6px 0 4px" }}>
                {m.value}
              </div>
              ...
              upcomingEvents.slice(0, 5).map((event, i) => (
                <div
                  ...
                  style={{
                    ...
                    gap: 12,
                    padding: "10px 0",
                    borderBottom: i < upcomingEvents.slice(0, 5).length - 1 ? "1px solid #f5f5f5" : "none",
                  }}
                >
                  <div
                    style={{
                      ...
                      display: "flex", alignItems: "center", justifyContent: "center",
                      color: C.blue, fontSize: 18, flexShrink: 0,
                    }}
                  >
                    <CalendarOutlined />
                  </div>
                  ...
                  <Tag color="purple" style={{ borderRadius: 6, fontSize: 11, fontWeight: 500 }}>
                    {event?.category}
                  </Tag>
                </div>
              ))
            ...
            {validEvents.length === 0 ? (
              ...
              validEvents.slice(0, 6).map((event, i) => {
                const eventBookings = Array.isArray(bookings)
                  ? bookings.filter((b) => b?.eventId === event?.id || b?.id === event?.id).length
                  : 0;
                const guests = event?.number_of_guests || 100;
                const pct = Math.min(100, Math.round((eventBookings / guests) * 100));
                return (
                  <div key={event?.id ?? i} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    <span
                      style={{
                        width: 110, fontSize: 12, color: "#555", flexShrink: 0,
                        whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
                      }}
                    >
                      {event?.title}
                    ...
                    <Progress
                      percent={pct}
                      showInfo={false}
                      strokeColor={pct > 70 ? C.green : pct > 40 ? C.blue : C.gold}
                      trailColor="#f5f5f5"
                      style={{ flex: 1, marginBottom: 0 }}
                    />
                    <span style={{ width: 32, fontSize: 12, fontWeight: 700, color: "#555", textAlign: "right" }}>
                      {pct}%
                    </span>
                  </div>
                );
            ...
            {validEvents.length === 0 ? (
              ...
              return sorted.map(([org, count], i) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: "#1a1a2e" }}>{org}</div>
                    <Progress
                  ...
                  <Tag
                    color={catColors[i % catColors.length] === C.red ? "red" : "default"}
                    style={{
